...
<script>
  (function() {
    ...
    if (phpUser) {
      ...
      const userFavs = localStorage.getItem(`angelow_favorites_${phpUser.email}`) || localStorage.getItem(`angelow_favorites_${phpUser.id}`);
...
<header>
  ...
  <div class="header-icons">
    ...
    <div class="icon-btn icon-btn-repartidor">
      <a href="/repartidor/login" target="_blank" title="Ser Repartidor" style="display: flex; align-items: center; justify-content: center; color: white; font-size: 20px; text-decoration: none; width: 100%; height: 100%;">
        <img src="<?= APP_URL ?>/assets/imagenes/general/motico.png" alt="Repartidor" style="width: 22px; height: 22px; filter: brightness(0) invert(1);">
      </a>
    </div>
...
<section class="flash-deals-section" id="flashDealsSection">
  ...
  <p class="section-sub">Stock limitado con tiempo descendente por producto</p>
  <div class="offers-grid" id="dynamicOffersGrid"></div>
  
  <!-- Widget interactivo: Scratch para descuento -->
  <div class="scratch-card">
    <h3>Rasca y gana descuento</h3>
