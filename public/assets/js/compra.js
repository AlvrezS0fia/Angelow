...
function validarPaso1() {
    ...
    if (!cedula.value.trim() || !validarCedula(cedula.value)) {
...
function validarPaso2() {
    ...
    if (!depto.value.trim()) {
        depto.classList.add('error');
        valido = false;
    } else {
        depto.classList.remove('error');
    }
    ...
    if (!carrito || carrito.length === 0) {
        contenedor.innerHTML = `
            <div class="empty-cart" style="text-align: center; padding: 40px 20px; color: #6b7280;">
                ...
                <a href="http://localhost/Angelow/public/" style="display: inline-block; margin-top: 15px; padding: 10px 24px; background: #1e3a8a; color: white; border-radius: 8px; text-decoration: none; font-weight: 600;">Ir a la tienda</a>
            </div>
        `;
        actualizarTotales();
        return;
...
function generarPDF(data) {
    const facturaElement = document.getElementById('facturaContainer');
    ...
    if (typeof html2pdf === 'undefined') {
        showToast('Error: Biblioteca PDF no cargada', 'error');
        return;
    }
    
    html2pdf().set(opt).from(facturaElement).save().then(() => {
...
function guardarPedido(facturaData) {
    ...
    const nuevoPedido = {
        ...
        products: facturaData.productos,
    ...
    console.log('Pedido guardado correctamente:', nuevoPedido);
    return nuevoPedido;
}
...
window.completePurchase = function() {
    ...
    const numeroFactura = `INV-${Date.now()}`;
...
function mostrarFactura(data) {
    ...
    const logoUrl = 'http://localhost/Angelow/public/assets/imagenes/general/logos.png';
    
    const facturaHTML = `
        <div id="facturaContainer" style="max-width: 820px; margin: 0 auto; background: #ffffff; border-radius: 16px; box-shadow: 0 10px 40px rgba(30, 58, 138, 0.12); overflow: hidden; padding: 50px; border: 1px solid #e8edf5;">
            ...
            <div style="text-align: center; border-bottom: 3px solid #1e3a8a; padding-bottom: 25px; margin-bottom: 30px;">
                <div style="display: flex; justify-content: center; align-items: center; gap: 20px; flex-wrap: wrap;">
                    ...
                    <div>
                        <h1 style="color: #1e3a8a; font-size: 28px; margin: 0; font-weight: 700; letter-spacing: -0.5px;">ANGELOW</h1>
                        <span style="color: #4a6fa5; font-size: 12px; letter-spacing: 3px; text-transform: uppercase; font-weight: 500;">Moda Infantil</span>
                    </div>
                </div>
                <div style="margin-top: 15px;">
                    <span style="color: #1e3a8a; font-size: 18px; font-weight: 700; letter-spacing: 2px; border: 2px solid #1e3a8a; padding: 5px 25px; border-radius: 30px;">FACTURA</span>
                </div>
                <div style="margin-top: 10px; color: #4a6fa5; font-size: 14px; font-weight: 500;">N° ${data.numero}</div>
                <div style="color: #6b7280; font-size: 13px;">${data.fecha}</div>
            </div>
            
            <div style="text-align: center; background: #f0f4fa; border-radius: 10px; padding: 12px; margin-bottom: 25px;">
                <span style="color: #1e3a8a; font-size: 15px; font-weight: 600;">Gracias por tu compra, ${data.cliente.nombre}</span>
            </div>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px; margin-bottom: 25px; background: #f8fafc; padding: 20px 25px; border-radius: 12px; border: 1px solid #e8edf5;">
                <div>
                    <h3 style="color: #1e3a8a; font-size: 13px; margin: 0 0 10px 0; text-transform: uppercase; letter-spacing: 2px; font-weight: 600; border-bottom: 2px solid #1e3a8a; padding-bottom: 6px;">Datos del Cliente</h3>
                    <p style="margin: 5px 0; color: #1f2937; font-weight: 600; font-size: 14px;">${data.cliente.nombre}</p>
                    <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">CC: ${data.cliente.cedula}</p>
                    <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">${data.cliente.telefono}</p>
                    <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">${data.cliente.email}</p>
                </div>
                <div>
                    <h3 style="color: #1e3a8a; font-size: 13px; margin: 0 0 10px 0; text-transform: uppercase; letter-spacing: 2px; font-weight: 600; border-bottom: 2px solid #1e3a8a; padding-bottom: 6px;">Datos de Envio</h3>
                    <p style="margin: 5px 0; color: #1f2937; font-weight: 600; font-size: 14px;">${data.envio.destinatario}</p>
                    <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">${data.envio.direccion}</p>
                    <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">${data.envio.metodo}</p>
                    <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">${data.envio.costo > 0 ? `COP $${data.envio.costo.toLocaleString()}` : 'Gratis'}</p>
                </div>
            </div>
            
            <div style="margin-bottom: 25px; overflow-x: auto; border-radius: 12px; border: 1px solid #e8edf5;">
                <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                    <thead>
                        <tr style="background: #1e3a8a;">
                            <th style="padding: 12px 16px; text-align: left; color: #ffffff; font-weight: 600;">Producto</th>
                            <th style="padding: 12px 16px; text-align: center; color: #ffffff; font-weight: 600;">Talla</th>
                            <th style="padding: 12px 16px; text-align: center; color: #ffffff; font-weight: 600;">Cant.</th>
                            <th style="padding: 12px 16px; text-align: right; color: #ffffff; font-weight: 600;">Precio Unit.</th>
                            <th style="padding: 12px 16px; text-align: right; color: #ffffff; font-weight: 600;">Subtotal</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${data.productos.map((p, index) => `
                            <tr style="border-bottom: 1px solid #e8edf5; ${index % 2 === 0 ? 'background: #f8fafc;' : 'background: #ffffff;'}">
                                <td style="padding: 12px 16px; color: #1f2937; font-weight: 500;">${p.nombre}</td>
                                <td style="padding: 12px 16px; text-align: center; color: #4b5563;">${p.talla}</td>
                                <td style="padding: 12px 16px; text-align: center; color: #4b5563;">${p.cantidad}</td>
                                <td style="padding: 12px 16px; text-align: right; color: #4b5563;">COP $${p.precioUnitario.toLocaleString()}</td>
                                <td style="padding: 12px 16px; text-align: right; color: #1e3a8a; font-weight: 600;">COP $${p.subtotal.toLocaleString()}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
            ...
            <div style="display: flex; justify-content: flex-end; background: #f8fafc; border-radius: 12px; padding: 20px 25px; margin-bottom: 25px; border: 1px solid #e8edf5;">
                <div style="width: 300px;">
                    <div style="display: flex; justify-content: space-between; padding: 5px 0; color: #4b5563; font-size: 14px;">
                        <span>Subtotal</span>
                        <span>COP $${data.subtotal.toLocaleString()}</span>
                    </div>
                    ${data.descuento > 0 ? `
                        <div style="display: flex; justify-content: space-between; padding: 5px 0; color: #10b981; font-size: 14px;">
                    ...
                    <div style="display: flex; justify-content: space-between; padding: 12px 0 6px 0; border-top: 3px solid #1e3a8a; margin-top: 6px;">
                        <span style="color: #1e3a8a; font-size: 18px; font-weight: 700;">TOTAL</span>
                        <span style="color: #1e3a8a; font-size: 22px; font-weight: 700;">COP $${data.total.toLocaleString()}</span>
                    </div>
                    <div style="text-align: right; margin-top: 8px; color: #6b7280; font-size: 12px;">${data.pago.metodo}</div>
                    ${data.infoAdicional ? `<div style="text-align: right; margin-top: 4px; color: #6b7280; font-size: 12px;">${data.infoAdicional}</div>` : ''}
                </div>
            </div>
            
            <div style="border-top: 2px solid #e8edf5; margin-top: 20px; padding-top: 20px; text-align: center; color: #6b7280; font-size: 12px;">
                <p style="margin: 0; color: #1e3a8a; font-size: 14px; font-weight: 500;">Gracias por elegir ANGELOW</p>
                <p style="margin: 5px 0 0 0; color: #4a6fa5;">Moda Infantil - Calidad y Estilo</p>
                <p style="margin: 5px 0 0 0; font-size: 11px; color: #9ca3af;">Factura N° ${data.numero} | ${data.fecha}</p>
            </div>
            
            <div style="display: flex; gap: 12px; justify-content: center; margin-top: 30px; flex-wrap: wrap;">
                <button onclick="generarPDFDatos()" style="background: #1e3a8a; color: white; border: none; padding: 14px 36px; border-radius: 50px; font-size: 14px; font-weight: 600; cursor: pointer; box-shadow: 0 4px 15px rgba(30, 58, 138, 0.3);">
                    Descargar PDF
                </button>
                <button onclick="window.location.href='http://localhost/Angelow/public/perfil'" style="background: #e8edf5; color: #1e3a8a; border: 2px solid #1e3a8a; padding: 14px 36px; border-radius: 50px; font-size: 14px; font-weight: 600; cursor: pointer;">
                    Ver Mis Pedidos
                </button>
                <button onclick="window.location.href='http://localhost/Angelow/public/'" style="background: #10b981; color: white; border: none; padding: 14px 36px; border-radius: 50px; font-size: 14px; font-weight: 600; cursor: pointer;">
                    Volver a la Tienda
                </button>
            </div>
        </div>
